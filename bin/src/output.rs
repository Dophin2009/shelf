...
// Utility for creating sl_i_* macros.
...
// Utility for creating sl_ii_* macros.
macro_rules! sl_ii_fmt {
    ($level:ident; $color:literal; [$($format_str:literal),+ $(,)?] $(, $arg:expr)* $(,)?) => {
        bunt_logger::$level!(["{:2}", "{$", $color, "}>{/$} ", "{:4}", $($format_str),+], "", "" $(, $arg )*)
    };
}

macro_rules! sl_ii_error {
    ($format_str:literal $(, $arg:expr)* $(,)?) => {
        sl_ii_error!([$format_str] $(, $arg )*)
    };
    ([$($format_str:literal),+ $(,)?] $(, $arg:expr)* $(,)?) => {
        sl_ii_fmt!(error; "red"; [$($format_str),+] $(, $arg )*)
    };
}

macro_rules! sl_ii_warn {
    ($format_str:literal $(, $arg:expr)* $(,)?) => {
        sl_ii_warn!([$format_str] $(, $arg )*)
    };
    ([$($format_str:literal),+ $(,)?] $(, $arg:expr)* $(,)?) => {
        sl_ii_fmt!(warn; "yellow"; [$($format_str),+] $(, $arg )*)
    };
}

macro_rules! sl_ii_info {
    ($format_str:literal $(, $arg:expr)* $(,)?) => {
        sl_ii_info!([$format_str] $(, $arg )*)
    };
    ([$($format_str:literal),+ $(,)?] $(, $arg:expr)* $(,)?) => {
        sl_ii_fmt!(info; "dimmed"; [$($format_str),+] $(, $arg )*)
    };
}

macro_rules! sl_ii_debug {
    ($format_str:literal $(, $arg:expr)* $(,)?) => {
        sl_ii_debug!([$format_str] $(, $arg )*)
    };
    ([$($format_str:literal),+ $(,)?] $(, $arg:expr)* $(,)?) => {
        sl_ii_fmt!(debug; "dimmed"; [$($format_str),+] $(, $arg )*)
    };
}

macro_rules! sl_ii_trace {
    ($format_str:literal $(, $arg:expr)* $(,)?) => {
        sl_ii_trace!([$format_str] $(, $arg )*)
    };
    ([$($format_str:literal),+ $(,)?] $(, $arg:expr)* $(,)?) => {
        sl_ii_fmt!(trace; "dimmed"; [$($format_str),+] $(, $arg )*)
    };
}
